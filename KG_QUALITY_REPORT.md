...
本报告记录当前知识图谱的质量边界。它不宣称项目已经全量完成，只说明哪些关系可以采信、哪些关系仍需复核，以及哪些数字来自同一发布快照。
...
| `assets/data/world500/workbench/snapshot_manifest.json` | 图层 hash、节点数、边数、公司数和计数口径 |
...
| `docs/REQUIREMENT_TRACEABILITY.md` | 内部交付闭环追踪矩阵 |
...
GHGP 已从泛化 `GHG Protocol` 拆成 12 个细分标准、指南和项目协议节点。accepted 边只保留页级原文明示具体系列的关系。不能命名具体系列的泛化引用被降级或排除出 accepted 图。

5 个零 accepted 标准不应被强行补边。若企业报告未明确点名这些政府、社区、项目或尚未正式发布的标准，应保留为“无 accepted 企业证据”或“适用范围不在企业披露主语境”。

### 3. 关系层

事实采信边和本体骨架边必须分开统计。企业-标准、企业-排放、企业-技术项目等事实边需要页级证据；标准族层级、节点分类和概念归属属于本体骨架，不进入 evidence-page 覆盖率分母。
...
图 6 已有 9 类技术路径和 77 条项目或措施级证据，但仍需严格区分关键词披露、项目证据、成本证据和减排效果证据。成本/投资强证据 11 条，是当前最需要继续补强的薄弱点。
