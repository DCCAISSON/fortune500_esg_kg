...
本文档把核心业务关注点映射到页面、图表、数据、口径说明和验证入口。它用于内部验收、门控检查和专家接手，不作为网站前台叙事主线，也不替代原始数据账本。

对外页面继续使用知识图谱项目语言：标准体系、排放排行、标准-企业图谱、技术路径、数据质量。内部追踪保留原始需求边界，目的是防止漏项和过度宣称。

权威数字以 `assets/data/world500/workbench/reporting_views.json`、`assets/data/world500/workbench/snapshot_manifest.json` 和相应 ledger 文件为准。不同图层的节点数、边数和公司数不得混写为同一快照。

## 统一快照指标

| 指标 | 当前值 | 权威来源 |
| --- | ---: | --- |
| GHGP accepted 细分边 | 267 | `world500_ghg_series_acceptance_ledger.json` |
| GHGP 泛化 accepted 引用 | 0 | `world500_ghg_series_acceptance_ledger.json` |
| GHGP 白名单标准数 | 12 | `world500_ghg_series_acceptance_ledger.json` |
| 标准角色 accepted 边 | 456 | `reporting_views.json` |
| 标准角色 review 排除边 | 1452 | `reporting_views.json` |
| 完整可比排放排行企业 | 27 | `world500_emissions_ranking_evidence_ledger.json` / `reporting_views.json` |
| 可用排放排行企业 | 91 | `reporting_views.json` |
| 技术路径项目证据 | 77 | `reporting_views.json` |
| 项目级成本/投资强证据 | 11 | `reporting_views.json` |
| 项目级减排效果证据 | 68 | `reporting_views.json` |
| 原文明示 primary-data 百分比企业 | 13 | `reporting_views.json` |

## 当前总判断

| 模块 | 当前状态 | 可以对外声明 | 主要风险 |
| --- | --- | --- | --- |
| GHG Protocol 细分标准和企业关联 | 部分完成：已建模 12 个 GHGP 节点，accepted 细分边 267 | 可以声明已建立 accepted/review/demoted 闭环，不能声明全量完成 | 仍有 demoted/review 历史队列和弱证据上下文需要继续清洗 |
| 企业总碳排放量排序 | 部分完成：完整可比排放排行 27 家 | 可以展示强证据主榜，partial 只能作参考 | Scope 1/2/3 缺口企业不能混入主排行 |
| 标准角色族全屏实体级知识图谱 | 展示层已实现，证据边界仍需持续复核 | 可以作为 accepted 验收图展示 | 非 GHG 语境中的 Scope 词汇只能作为原文引用，不能扩展成标准口径 |
| 图 6 技术路径主轴 | 部分完成：项目证据 77 条，成本/投资强证据 11 条 | 可以展示“披露信号 + 项目证据”分层 | 技术关键词不能等同于已实施项目，成本证据仍薄弱 |
| 初级/次级数据气泡图 | 部分完成：13 家有原文明示 primary-data 百分比 | 可以作为数据质量探索图展示 | 推断比例不能当作审计级计算权重 |

## GHG Protocol 具体系列、原则和企业关联

### 模块目标

展示企业具体引用了哪些 GHG Protocol 标准、指南或项目协议，而不是把所有引用合并为一个泛化的 `GHG Protocol` 大类。

### 当前交付物

| 类型 | 路径 |
| --- | --- |
| GHGP 全屏验收图 | `zh/ghg-protocol-full-graph.html`、`en/ghg-protocol-full-graph.html` |
| 标准角色全屏图 | `zh/role-family-standard-full-graph.html`、`en/role-family-standard-full-graph.html` |
| 核心发布视图 | `assets/data/world500/workbench/reporting_views.json` |
...
当前完整可比主榜为 27 家。若汇报强依赖“从高到低”结论，应继续把 near-complete 企业补成 Scope 1/2/3 闭环，目标达到 30 家以上；但不得用 partial total 填充主榜。
...
图 6 必须区分五层证据：关键词披露、公司层面技术提及、项目或措施证据、成本或投资证据、减排效果证据。只有项目或措施证据以上才能支持“企业有具体减碳项目”的表述；成本或投资证据不能用容量、采购量或项目规模替代。
